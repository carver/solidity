...
{
public:
	explicit StackCompressor(std::shared_ptr<Dialect> _dialect): m_dialect(std::move(_dialect)) {}
	/// Try to remove local variables until the AST is compilable.
	/// @returns true if it was successful.
	bool run(Block& _ast);

private:
	void eliminateVariables(FunctionDefinition& _function, size_t _numVariables);
